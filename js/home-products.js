...
(function () {
  ...
  function moneyNGNFromKobo(priceKobo) {
    const amount = Number(priceKobo || 0) / 100;
  ...
  function usdCentsToKobo(usdCents, rate) {
    // matches your backend logic: kobo = usdCents * rate
  ...
  // backend static uploads (DO NOT add /api)
  return `${API_BASE}${url}`;
}
  ...
  function cardHTML(p, wishlistIds) {
    ...
    // Prefer backend priceKobo if present, else use computedKobo
    const kobo = Number(p.priceKobo || p.__priceKobo || 0);
    ...
    return `
      <div class="rounded-2xl border border-slate-200 bg-white shadow-sm overflow-hidden hover:shadow-md transition">
        <div class="relative">
          ...
          <!-- Wishlist -->
          <button
            class="absolute right-3 top-3 h-10 w-10 grid place-items-center rounded-full border border-slate-200 bg-white/95 hover:bg-slate-50 transition ${
          ...
          >
            <i class="${
              active ? "fa-solid fa-heart text-rose-600" : "fa-regular fa-heart text-slate-700"
            }"></i>
          </button>
        </div>
        ...
        <div class="p-5 space-y-3">
          ...
          <button
            ...
            data-pricekobo="${escapeHtml(String(kobo))}"
            data-currency="NGN"
            type="button"
          >
  ...
  async function init() {
    try {
      ...
      // 2) Ensure we have NGN priceKobo (if backend didn’t send it)
      let rate = 0;
      try {
        const fx = await api("/fx/usd-ngn");
        rate = Number(fx?.rate || 0);
      } catch {
        rate = 0;
      }

      const normalized = top3.map((p) => {
        // if backend already provides priceKobo, keep it
        if (p?.priceKobo != null) return p;

        // else compute from USD cents
        const usdCents = Number(p?.priceUsdCents || 0);
        const kobo = rate > 0 && usdCents > 0 ? usdCentsToKobo(usdCents, rate) : 0;

        return { ...p, __priceKobo: kobo };
      });

      const wishlistIds = await getWishlistIds();

      empty?.classList.add("hidden");
      grid.innerHTML = normalized.map((p) => cardHTML(p, wishlistIds)).join("");
