// js/cart-page.js
import { getCart, updateQty, removeItem, cartTotalKobo } from "./cart.js";
...
function render() {
  ...
  if (!cart.length) {
    ...
    subtotalEl.textContent = moneyNGN(0);
    totalEl.textContent = moneyNGN(0);
  ...
  const totalKobo = cartTotalKobo();
  subtotalEl.textContent = moneyNGN(totalKobo);
  totalEl.textContent = moneyNGN(totalKobo);

  list.innerHTML = cart
    .map((item) => {
      const qty = Number(item.quantity || 1);
      const priceKobo = Number(item.priceKobo || 0);
      const lineKobo = priceKobo * qty;
          ...
          <div class="flex-1 min-w-0">
            <div class="flex items-start justify-between gap-3">
              <div class="min-w-0">
                ...
                <p class="mt-1 text-sm text-slate-600">${moneyNGN(priceKobo)}</p>
                ${
                  priceKobo <= 0
            ...
            <div class="mt-4 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
              ...
              <div class="text-sm">
                ...
                <span class="font-bold text-slate-900">${moneyNGN(lineKobo)}</span>
              </div>
            </div>
          </div>
        </div>
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  window.addEventListener("storage", (e) => {
    if (e.key === "foodcheq_cart_v1") render();
  });
});
