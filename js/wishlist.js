...
// Pull from server into local KEY (only if logged in)
export async function syncWishlistFromServer() {
  const token = getToken?.() || "";
  if (!token) return readIdsArray();

  try {
    const res = await api("/wishlist/ids", { auth: true });
...
export async function addToWishlist(productId) {
  ...
  if (token) {
    try {
      await api("/wishlist", { method: "POST", auth: true, body: { productId: id } });
      // keep local as is
    } catch (e) {
...
export async function removeFromWishlist(productId) {
  ...
  if (token) {
    try {
      await api(`/wishlist/${encodeURIComponent(id)}`, { method: "DELETE", auth: true });
    } catch (e) {
      // rollback if server rejected
