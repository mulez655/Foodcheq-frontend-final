...
(function () {
  ...
  // ✅ Compute product price in kobo for UI/cart/sort
  function getProductPriceKobo(p) {
    const direct = Number(p?.priceKobo || 0);
    if (direct > 0) return direct;

    const usdCents = Number(p?.priceUsdCents || 0);
    if (usdCents > 0) {
      const rate = Number(state.usdNgnRate || 0) || 1600;
      return usdCentsToKobo(usdCents, rate);
    }
  ...
  function applyFilters() {
    ...
    filtered.sort((a, b) => {
      ...
      if (s === "low-high") return getProductPriceKobo(a) - getProductPriceKobo(b);
      if (s === "high-low") return getProductPriceKobo(b) - getProductPriceKobo(a);
      return 0;
    });
  ...
  function cardHTML(p, wishlistIds) {
    ...
    const priceKobo = getProductPriceKobo(p);
    ...
    return `
      <article class="group relative rounded-2xl border border-slate-200 bg-white overflow-hidden shadow-sm hover:shadow-md transition">
        <!-- Wishlist -->
        <button
          class="absolute top-3 right-3 z-10 h-10 w-10 grid place-items-center rounded-full border border-slate-200 bg-white/95 backdrop-blur hover:bg-slate-50 transition
        ...
        <div class="px-4 pb-4">
          <button
            ...
            data-pricekobo="${escapeHtml(priceKobo)}"
            data-currency="NGN"
          >
  ...
  async function init() {
    try {
      ...
      // ✅ Load FX once (cached by fx.js)
      state.usdNgnRate = await getUsdNgnRate();
